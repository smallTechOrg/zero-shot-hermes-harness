...
You are the ROOT SESSION orchestrating a spec↔code sync by running the specialist roles in `../../support/agents/` — via `delegate_task` when available, **inline otherwise** (read the role file as a checklist). Verify every handback. **Spec is the source of truth — when spec and code disagree, fix the code** (../../support/patterns/spec-driven.md). Optional scope in `$ARGUMENTS`; otherwise the whole project. Run autonomously to a CLEAN audit; pause only on a hard blocker or if a divergence reveals the *spec* is wrong (surface it — don't silently rewrite the spec to match code).
