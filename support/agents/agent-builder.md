...
- `support/rules/ai-agents.md` — session rules, the build flow, real-key testing discipline
- `support/patterns/phases.md` — phase model and per-phase gates
- `support/rules/git.md` — branch/PR/commit-push discipline (you own git, so follow this exactly)
- `support/rules/secret-hygiene.md` — never commit secrets; `.env` stays untracked
...
2. Create the project directories per `support/patterns/project-layout.md`. Never write app code at the repo root.
...
4. First commit (scaffold) + push, then open the PR immediately — a PR must exist before the first feature commit (`support/rules/git.md`). **Base it on `<base>`, not `main`**, so the PR shows only the generated app: `gh pr create --base "$base" --head feature/<slug>-v0.1`.
